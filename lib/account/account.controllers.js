module.exports = (models) => {
  ...
  return {
    ...
    //TODO: Limit route to superadmin
    ...
    //TODO: Limit route to superadmin or user matching the id
    ...
    patchAccount: async function(request, h) {
      ...
      // Check to see that a user has the proper account credentials
      try {
        currentAccount = await getCurrentAccount(credentials);
      } catch (e) {
        throw Boom.badImplementation('Error during patchAccount - getCurrentAccount(credentials). ' + e);
      }
      // Check to see that an account exists for the retrieved credentials
      if (currentAccount === null) {
        throw Boom.conflict("Account does not exist for currently provided credentials");
      }
      // Check to see that the retrieved account matches the one being patched
